...
- WebSocket in Node.js using the `ws` npm package, with header authentication by default
...
```ts
import { NeuroInfoApiClient } from "@neuroinfoapi-client/node";
...
```ts
import { NeuroInfoApiClient, NeuroInfoApiWebsocketClient } from "@neuroinfoapi-client/node";
...
This package defaults to `authMethod: "header"` in Node.js. Set `authMethod: "ticket"` to obtain a one-time ticket through HTTP before connecting. For browser usage, use `@neuroinfoapi-client/native`.
...
```ts
import { NeuroInfoApiEventer } from "@neuroinfoapi-client/node";
...
```ts
import { NeuroInfoApiWebsocketClient } from "@neuroinfoapi-client/node";
